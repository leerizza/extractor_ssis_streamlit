...
class SSISMetadataExtractor:
    ...
    def parse_sql_column_sources(self, sql_query):
        ...
        try:
            ...
            sql_no_comment = re.sub(r'/\*.*?\*/', '', sql_query, flags=re.DOTALL)
            sql_no_comment = re.sub(r'--.*$', '', sql_no_comment, flags=re.MULTILINE)
            sql_clean = sql_no_comment.upper().strip()
            
            # PHASE 1: EXTRACT CTEs (WITH clause)
            # Pattern: WITH cte1 AS (...), cte2 AS (...) SELECT...
            subquery_mappings = {} # alias -> {col -> source}
            
            # Check for WITH clause at the start
            with_match = re.match(r'^\s*WITH\s+', sql_clean, re.IGNORECASE)
            
            if with_match:
                # We have CTEs to process
                # Find the main SELECT by looking for SELECT not inside parentheses
                # Strategy: Extract all CTEs first, then process main query
                
                cte_section_start = with_match.end()
                
                # Find where CTEs end and main SELECT begins
                # We need to find the SELECT that's at the same parenthesis level as WITH
                paren_depth = 0
                main_select_start = -1
                ...
                for i, char in enumerate(sql_clean[cte_section_start:], start=cte_section_start):
                    if char == '(':
                        ...
                        # Check if we hit SELECT at top level
                ...
                if main_select_start != -1:
                    ...
                    # Parse individual CTEs from the section
                    ...
                    while True:
                        ...
                        open_paren_pos = cte_start_pos + cte_match.end() - 1  # Position of '('
                        ...
                        if close_paren_pos != -1:
                            ...
                            # Recursively parse the CTE
                            subquery_mappings[cte_alias] = self.parse_sql_column_sources(cte_sql)
                            
                            # Move past this CTE (look for comma or end)
                            cte_start_pos = close_paren_pos + 1
                        else:
                            break
                    
                    # Update sql_clean to be just the main SELECT (without WITH clause)
            ...
            for col in columns:
                ...
                if '=' in col and not (col.startswith("'") or 'CASE ' in col.upper()):
                     parts = col.split('=', 1)
    ...
    def get_package_info(self):
        ...
        return {
            ...
            'Creation Date': self.root.get(f'{ns}CreationDate', 'N/A'),
            'Creator': self.root.get(f'{ns}CreatorName', 'N/A'),
            'Computer': self.root.get(f'{ns}CreatorComputerName', 'N/A'),
    ...
    def _trace_column_lineage_topology(self):
        """Holy Grail: Topological Tracing using LineageIDs"""
        lineage_results = []
        ...
        for dft in self._get_dataflow_tasks():
            ...
            # 2. Lineage Map: LineageID -> SourceInfo (Dict)
            ...
            while queue:
                ...
                # B. Transformations (Pass-through vs Async)
                    ...
                    # Logic: 
                    # 1. Identify Input Columns and their Upstream Source
                    # 2. Identify Output Columns
                    #    - If Synchronous (share LineageID with Input), inherit Source info.
                    #    - If Asynchronous (New LineageID), try to map input -> output (Union) or mark as Derived.
                    
                    # Map: InputLineageID -> UpstreamInfo
                    input_map = {}
                    for inp in comp.findall('.//input', {}):
                        for col in inp.findall('.//inputColumn', {}):
                            lid = col.get('lineageId')
                            if lid in lineage_id_map:
                                input_map[lid] = lineage_id_map[lid]
                    ...
                    for output in comp.findall('.//output', {}):
                        ...
                        for out_col in output.findall('.//outputColumn', {}):
                            ...
                            # Case 1: Existing LineageID (Pass-through)
                            # Note: In XML, synchronous columns might NOT be listed in outputColumn if unmodified?
                            # Actually, SSIS XML lists them if they have properties.
                            # But often the LineageID originates from upstream and just flows.
                            # So even if not in outputColumn loop, it exists in our lineage_id_map!
                            # WE DO NOT NEED TO DO ANYTHING for pass-through IDs that are unchanged numbers.
                            # They already exist in lineage_id_map.
                            
                            # Case 2: New Lineage ID (Derived or Async)
                            # But wait, does Derived Column create NEW LineageID for REPLACED columns?
                            # Usually creates new ID and marks 'replace'.
                            
                            # Check if this LID is already known?
                            if lid in lineage_id_map:
                                # Start with existing info (e.g. from upstream)
                                info = lineage_id_map[lid].copy()
                                # Update if transformed?
                            else:
                                # New ID. Need to derive source.
                                if 'UnionAll' in comp_class:
                                    # Union All is tricky. It has 1 Output.
                                    # Output Column X maps to Input 1 Col A, Input 2 Col B.
                                    # SSIS Metadata: OutputColumn usually doesn't say.
                                    # We need to look at 'inputs'.
                                    # InputColumn has 'refId'?? No.
                                    # Standard SSIS Union: matched by ORDER.
                                    # Let's verify: Union All maps nth column of Input K to nth column of Output.
                                    # We can try to map by Index?
                                    
                                    # Get index of this output column
                                    out_cols = list(output.findall('.//outputColumn', {}))
                                    try:
                                        idx = out_cols.index(out_col)
                                        # Find sources from all input(s) at this index?
                                        # Union merges multiple sources.
                                        sources = set()
                                        src_tables = set()
                                        exps = set()
                                        ...
                                        for inp in comp.findall('.//input', {}):
                                            ...
                                            if idx < len(in_cols):
                                                ...
                                                if in_lid in lineage_id_map:
                                                    upstream = lineage_id_map[in_lid]
                                                    src_tables.add(upstream['Source Table'])
                                                    # exps.add(upstream['Expression/Logic']) # too noisy
                                        
                                        lineage_id_map[lid] = {
                                            'Source Component': 'Union (Merged)',
                                            'Source Table': ', '.join(sorted(src_tables)) if src_tables else 'Unknown',
                                            'Original Column': out_col.get('name'),
                                            'Expression/Logic': 'Union',
...
# File uploader
def render_package_details(extractor):
    ...
    tab1, tab2, tab3, tab4, tab5, tab_qual, tab6, tab7, tab8 = tabs
...
if source_mode == "Upload Files":
    ...
    if uploaded_files:
        for f in uploaded_files:
            packages_to_process.append((f.name, f.read().decode('utf-8')))
                ...
                for f in files:
                    ...
                    try:
                        with open(full_path, 'r', encoding='utf-8') as file:
                            packages_to_process.append((f, file.read()))
                    except Exception as e:
...
if packages_to_process:
    ...
    try:
        with st.spinner("Extracting metadata..."):
            for fname, content in packages_to_process:
        ...
        if selected_pkg_name:
            ...
            render_package_details(selected_pkg['extractor'])
